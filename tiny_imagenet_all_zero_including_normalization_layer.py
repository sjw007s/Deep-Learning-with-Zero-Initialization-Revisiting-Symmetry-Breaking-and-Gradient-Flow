...
class PreNormResidual(nn.Module):
    def __init__(self, dim, fn):
        ...
        torch.nn.init.zeros_(self.norm.weight)
        torch.nn.init.zeros_(self.norm.bias)

    def forward(self, x):
        x1 = self.norm(x)
        if torch.count_nonzero(self.norm.weight) == 0:
            fan_in = x1.shape[-1]
            bound = 1 / math.sqrt(fan_in) if fan_in > 0 else 0
            x1 = x1 + torch.empty(x1.shape[-1], device=device).uniform_(-bound, bound)
        return self.fn(x1) + x
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        print(summary)
    with open("tiny_imagenet_all_zero_including_normalization_layer.csv", mode="w", newline="", encoding="utf-8") as file:
